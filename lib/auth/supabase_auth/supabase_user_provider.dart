...
class InsulinTrackerProjectSupabaseUser extends BaseAuthUser {
  InsulinTrackerProjectSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> insulinTrackerProjectSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = InsulinTrackerProjectSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
